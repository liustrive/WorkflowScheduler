...
public class ParamChecker {
    ...
    public static <T> List<T> notNullElements(List<T> list, String name) {
        ...
        for (int i = 0; i < list.size(); i++) {
            notNull(list.get(i),  name); // this is changed Liu
        }
        return list;
    }
    ...
    public static String validateActionName(String actionName) {
        ...
        if (actionName.length() > MAX_NODE_NAME_LEN) {
            throw new IllegalArgumentException("name [{0}] must be {1} chars or less"+ actionName);
        }

        char c = actionName.charAt(0);
        if (!(c >= 'A' && c <= 'Z') && !(c >= 'a' && c <= 'z') && !(c == '_')) {
            throw new IllegalArgumentException("name [{0}], must start with [A-Za-z_]"+actionName);
        }
        for (int i = 1; i < actionName.length(); i++) {
            c = actionName.charAt(i);
            if (!(c >= '0' && c <= '9') && !(c >= 'A' && c <= 'Z') && !(c >= 'a' && c <= 'z')
                    && !(c == '_' || c == '-')) {
                throw new IllegalArgumentException("name [{0}] must be [A-Za-z_][0-9A-Za-z_]*"+actionName);
            }
        }
        return actionName;
    }
    ...
    public static int checkGTZero(int value, String name) {
        if (value <= 0) {
            throw new IllegalArgumentException("parameter [{0}] = [{1}] must be greater than zero"+name+
                                                           value);
        }
        return value;
    }
    ...
    public static int checkInteger(String val, String name) {
        ...
        try {
            ...
            throw new IllegalArgumentException(
                    "parameter [{0}] = [{1}]  must be an integer. Parsing error {2}"+name+ val+ nex.getMessage() );
        }
        return ret;
    }
...
//    }
    ...
    public static boolean isMember(String item, String[] members, String name) {
        ...
        throw new IllegalArgumentException("parameter [{0}] = [{1}] " + "must be in the list {2}"+ name+
                                                       item+ buff.toString());
    }
}