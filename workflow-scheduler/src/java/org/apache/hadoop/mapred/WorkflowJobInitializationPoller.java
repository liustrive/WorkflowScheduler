 ...
 * Map/Reduce cluster running with the {@link WorkflowTaskScheduler}.
 *
 * <p>
...
public class WorkflowJobInitializationPoller extends Thread {
  ...
  class JobInitializationThread extends Thread {
    ...
    void addQueue(String queueName) {
      WorkflowSchedulerQueue queue = jobQueueManager.getQueue(queueName);
  ...
  public WorkflowJobInitializationPoller(WorkflowJobQueuesManager mgr,
      WorkflowSchedulerConf rmConf, Set<String> queue, 
  ...
  void init(int numQueues, 
            WorkflowSchedulerConf WorkflowConf) {
    sleepInterval = WorkflowConf.getSleepInterval();
    poolSize = Math.min(WorkflowConf.getMaxWorkerThreads(), numQueues);
